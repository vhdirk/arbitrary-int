use core::{
    ...
    hash::Hash,
    iter::{Product, Sum},
    num::{FpCategory, IntErrorKind, ParseIntError, Wrapping},
    ops::{
...
use crate::{util::{Assert, IsTrue}, Number, UInt};
...
macro_rules! uint_impl_funty_numeric_bits {
...
//                 // let mut bytes = [0u8; ($bits + 7) / 8];
//                 // let mut mask = Self::MASK;
//                 // for i in 0..bytes.len() {
//                 //     bytes[i] = (mask & 0xFF) as u8; // Take the lowest 8 bits
//                 //     mask >>= 8;                     // Shift right by 8 to get the next byte in little-endian order
//                 // }

//                 // bytes

//                 // Convert the value to bytes in little-endian order
//                 let all_bytes = self.value().to_le_bytes();

//                 // Initialize an array with the exact size needed
//                 let mut result = [0u8; Self::BYTES];

//                 // Copy only the necessary bytes from `all_bytes` into `result`
//                 let mut i = 0;
//                 while i < Self::BYTES {
//                     result[i] = all_bytes[i];
//                     i += 1;
//                 }
...
// // TODO: calculations in macros's are pretty limiting.
// // For now, I'm typing this all out but a modification of seq_macro that allows
// // expressions instead of just literales would be very nice
...
// macro_rules! uint_impl_funty_numeric {

//     ($type:ident, $bytes:expr, $bits:expr) => {

//     };

//     // This case recursively generates the implementations for each bit size
//     ($type:ident, $max_bits:expr) => {
//         // const_for!(num_bytes in 1..$max_bytes => {

//         //     // const_for!(num_bits in 1..{num_bytes*8} => {
//         //     //     uint_impl_funty_numeric!($type, num_bytes, num_bits);
//         //     // });
...
// uint_impl_funty_numeric!(u8, 1);

// macro_rules! uint_impl_funty_integral {
//     ($type:ident) => {
...
//     ($($type:ident, $max_bits:expr),+) => {
//         $(
//             uint_impl_funty_fundamental!($type);
//             uint_impl_funty_numeric!($type, $max_bits);
//             uint_impl_funty_integral!($type);
//         )+
//     };
// }
